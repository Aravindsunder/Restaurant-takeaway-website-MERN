...
export const Menu = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch food items
        const itemsResponse = await fetch("http://localhost:5000/items");
        let itemsData = await itemsResponse.json();
        itemsData = itemsData.filter((item) => item.status !== "draft");
        ...
        const userResponse = await fetch(
          `http://localhost:5000/users/${encodeURIComponent(userEmail)}`
        );
  ...
  const updateCartOnServer = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/users/${encodeURIComponent(userEmail)}/cart`,
        {
          method: "PUT",
          headers: {
